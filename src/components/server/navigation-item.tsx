...
const NavigationItem = ({ server }: NavigationItemProp) => {
  ...
  return (
    <ActionTooltip label={server.name} align="center" side="right" isActive={isActive}>
      <div
        className={cn(
          "group w-[48px] h-[48px] relative transition-all mx-auto",
      ...
      >
        <Image
          ...
          alt="server picture"
          fill
          className={cn("rounded-full z-30", isActive && "-translate-y-[2px]")}
        />
        <span
          className={cn(
            ...
            isActive
              ? "w-[48px] h-[48px] -bottom-1/2 -translate-y-1/2 rounded-full"
              : "group-hover:w-[26px]"
          )}
        />
      </div>
